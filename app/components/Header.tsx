...
export function Header() {
  ...
  const navigationTabs = Object.entries(navItems)
    ...
    .filter(
      ([path]) => path !== "/" && path.split("/").filter(Boolean).length === 1
    )
  ...
  return (
    ...
    <div className="flex justify-between items-center mb-8">
      ...
      <div className="space-y-3 flex-grow">
        {" "}
        {/* Added flex-grow to allow breadcrumbs/nav to take space */}
        {!isHomePage &&
          breadcrumbItems.length > 0 && ( // Render even if only Home icon is present
            <Breadcrumb>
              <BreadcrumbList>
